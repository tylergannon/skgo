...
// cspProvider ports kit's `BaseProvider` (`runtime/server/page/csp.js`) for
// the inline scripts and styles a skgo document can carry. Production links
// component CSS, while dev mirrors Kit by inlining the rendered branch's CSS
...
type cspProvider struct {
	...
	styleSrcBase      []string
	styleSrcElemBase  []string

	scriptSrcNeedsCSP     bool
	scriptSrcElemNeedsCSP bool
	scriptNeedsCSP        bool
	styleSrcNeedsCSP      bool
	styleSrcElemNeedsCSP  bool
	styleNeedsCSP         bool
...
func newCSPProvider(useHashes bool, directives map[string]CSPDirectiveValue, nonce string) *cspProvider {
	...
	styleSrcBase, ok := presentSources(directives, "style-src")
	if !ok {
		styleSrcBase, ok = presentSources(directives, "default-src")
	}
	styleSrcNeedsCSP := ok && cspNeedsCSP(styleSrcBase)
	styleSrcElemBase, elemOK := presentSources(directives, "style-src-elem")
	styleSrcElemNeedsCSP := elemOK && cspNeedsCSP(styleSrcElemBase)
	styleNeedsCSP := styleSrcNeedsCSP || styleSrcElemNeedsCSP
	...
	return &cspProvider{
		...
		styleSrcBase:          styleSrcBase,
		styleSrcElemBase:      styleSrcElemBase,
		styleSrcNeedsCSP:      styleSrcNeedsCSP,
		styleSrcElemNeedsCSP:  styleSrcElemNeedsCSP,
		styleNeedsCSP:         styleNeedsCSP,
		ScriptNeedsNonce:      scriptNeedsCSP && !useHashes,
		ScriptNeedsHash:       scriptNeedsCSP && useHashes,
		StyleNeedsNonce:       styleNeedsCSP && !useHashes,
		scriptSrc:             newOrdered(nil),
		scriptSrcElem:         newOrdered(nil),
		styleSrc:              newOrdered(nil),
		styleSrcElem:          newOrdered(nil),
	}
}
...
// AddStyle is Kit's `add_style`, called with the exact concatenated CSS bytes
// written into the dev document's `data-sveltekit` style element.
func (p *cspProvider) AddStyle(content string) {
	if !p.styleNeedsCSP {
		return
	}
	source := p.source(content)
	if p.styleSrcNeedsCSP {
		p.styleSrc.add(source)
	}
	if p.styleSrcElemNeedsCSP {
		p.styleSrcElem.add(source)
	}
}
...
func (p *cspProvider) header() string {
	...
	if p.styleSrc.items != nil {
		merged["style-src"] = CSPDirectiveValue{Sources: append(append([]string{}, p.styleSrcBase...), p.styleSrc.items...)}
	}
	if p.styleSrcElem.items != nil {
		merged["style-src-elem"] = CSPDirectiveValue{Sources: append(append([]string{}, p.styleSrcElemBase...), p.styleSrcElem.items...)}
	}
...
// AddStyle is Kit's `Csp.add_style`: both enforced and report-only providers
// record the same inline style.
func (d *documentCSP) AddStyle(content string) {
	d.provider.AddStyle(content)
	d.report.AddStyle(content)
}

// StyleNeedsNonce is true when either policy requires a nonce on an inline
// style element.
func (d *documentCSP) StyleNeedsNonce() bool {
	return d.provider.StyleNeedsNonce || d.report.StyleNeedsNonce
}
