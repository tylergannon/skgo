...
const unimplemented = (route: string): never => {
	if (building) throw new Error('skgo: route ' + route + ' is prerendered, and its branch has a Go server load at ' + "src/routes/(marketing)/pricing/page.server.go" + '; skgo cannot answer a load while kit prerenders (#81). Remove the prerender or move the load');
	throw new Error('skgo: implemented in Go');
};

export const load = (event: { url: URL }): { featured: { name: string; price: Money } } => unimplemented(event.url.pathname);
